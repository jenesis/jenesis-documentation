...
| `--java=<version>` | Run the program on an installed JDK matching the version, as `25`, `25.0.3` or `25-temurin`, which names Java 25 or newer: the JDK running jpx when it matches, otherwise the newest match in the folders `-Djenesis.toolchain.searchpath` names, the operating system's usual JDK locations by default. Nothing is installed unless `-Djenesis.toolchain.installer` names an installer, such as `jenesis-jdk`, for when none matches. The version and the search path follow the build tool's rules (see *[The JDK a build runs on](/tool/building-and-running/#the-jdk-a-build-runs-on)*). |

...
| `installation.home(home)` | The same installation launched with the JDK at `home` in place of the one running jpx; `--java` passes the home that `new Toolchain().version(version).home()` selects. |

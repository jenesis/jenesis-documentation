...
| `enableNativeAccess.<layer>` | The same for the modules of a [module layer](#module-layers). | yes, for what the module declaring the layer passes on with `@jenesis.layer <name> native` |
...
tool writes all three from the `@jenesis.native` declarations of the module it packages, and from the
`@jenesis.layer <name> native` lines of each module it grants - see

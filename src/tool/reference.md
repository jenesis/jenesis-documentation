...
| `jenesis.project.customizers` | *(none)* | Comma-separated `UnaryOperator<Project>` classes under `build/`, applied in order to the configured project (see *[Extending the build](/tool/extending-the-build/#adding-a-step-to-the-stock-pipeline)*). Command line or `~/.jenesis/jenesis.properties` only. |

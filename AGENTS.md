...
6. `npm run check` builds the site from scratch and crawls it, validating every internal link and every
   `#fragment` on every page the landing page reaches - which is every chapter, since each one is in its
   section's menu. It must pass before a push; the deploy runs only when it does.
